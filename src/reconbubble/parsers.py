...
def upsert_artifact(session: Session, type_: str, stored_path: Path) -> Artifact:
    ...
    art = Artifact(
        type=type_, filename=stored_path.name, stored_path=str(stored_path), sha256=h
    )
    session.add(art)
    session.commit()
    session.refresh(art)
...
def upsert_host(
    session: Session, ip: str, hostname: str = "", os_guess: str = ""
) -> Host:
...
def import_subdomains(session: Session, artifact: Artifact, path: Path) -> int:
    ...
    for line in path.read_text(encoding="utf-8", errors="ignore").splitlines():
        ...
        if not DOMAIN_RE.match(s):
            continue
        if session.scalar(select(Subdomain).where(Subdomain.fqdn == s)):
            continue
...
def import_emails(session: Session, artifact: Artifact, path: Path) -> int:
    ...
    for line in path.read_text(encoding="utf-8", errors="ignore").splitlines():
        ...
        s = f"{local}@{dom}"
        if session.scalar(select(Email).where(Email.email == s)):
            continue
...
def import_nmap_xml(session: Session, artifact: Artifact, path: Path) -> dict:
    ...
    for host in root.findall("host"):
        ...
        for p in host.findall("ports/port"):
            ...
            st = p.find("state")
            state_s = st.get("state", "unknown") if st is not None else "unknown"
            svc = p.find("service")
            ...
            if not db_svc:
                ...
                if service_name and not db_svc.service_name:
                    db_svc.service_name = service_name
                if product and not db_svc.product:
                    db_svc.product = product
                if version and not db_svc.version:
...
def import_document(session: Session, artifact: Artifact, path: Path) -> int:
    ...
    def _is_generic_doc_title(v: str) -> bool:
        t = (v or "").strip().lower()
        if not t:
            return True
        generic = {
    ...
    try:
        if path.suffix.lower() == ".pdf":
            ...
            if info:
                meta.update(
                    {k.strip("/"): str(v) for k, v in info.items() if v is not None}
                )
                if meta.get("Title"):
                    title = meta["Title"]
            ...
            if cp.title and not _is_generic_doc_title(cp.title):
                title = cp.title
    ...
    doc = Document(
        ...
        ),  # full exiftool+enriched metadata
    )
...
def import_prowl_phase1(session: Session, artifact: Artifact, path: Path) -> int:
    ...
    try:
        for i, (fqdn, info) in enumerate(domains_data.items()):
            ...
            prowl_ips_list = (
                list(set([ip for ip in prowl_ips_raw if ip])) if prowl_ips_raw else []
            )
            prowl_ips = ",".join(prowl_ips_list)
            prowl_registrar = info.get("registrar", "")
            prowl_netblocks_raw = info.get("netblocks", [])
            prowl_netblocks_list = (
                list(set([nb for nb in prowl_netblocks_raw if nb]))
                if prowl_netblocks_raw
                else []
            )
...
def import_smtp(session: Session, artifact: Artifact, path: Path) -> int:
    ...
    for item in results:
        ...
        # Check if already exists
        existing = session.execute(
            text("SELECT id FROM smtp_scans WHERE mx_host = :mx"), {"mx": mx_host}
        ).fetchone()
    ...
    for line in lines:
        ...
        if event_type == "DNS_NAME":
            ...
            if isinstance(data.get("data"), dict):
                description = data.get("data", {}).get("description", "") or data.get(
                    "data", {}
                ).get("technology", "")
            elif isinstance(data.get("data"), str):
                try:
                    description = json.loads(data.get("data", "{}")).get(
                        "description", ""
                    )
                except:
            ...
            if description and host:
                ...
                if DOMAIN_RE.match(host):
                    sub = session.scalar(
                        select(Subdomain).where(Subdomain.fqdn == host.lower())
                    )
                    if sub:
    ...
    for line in lines:
        ...
        if event_type == "DNS_NAME":
            ...
            if not provider:
                ...
                if (
                    "cloud" in tag_str.lower()
                    or "microsoft" in tag_str.lower()
                    or "azure" in tag_str.lower()
                    or "aws" in tag_str.lower()
                    or "gcp" in tag_str.lower()
                    or "google" in tag_str.lower()
                    or "amazon" in tag_str.lower()
                ):
                    provider = "other"
