...
def create_app(
    ...
    def scope_sets(s: Session):
        items = (
            s.execute(select(ScopeItem).where(ScopeItem.in_scope == 1)).scalars().all()
        )
    ...
    def assets(request: Request):
        ...
        if scoped_no_subnet > 0:
            subnet_stats_list.append(
                {"subnet": "Scoped IPs (No Subnet)", "count": scoped_no_subnet}
            )
    ...
    def _load_checklist_map(s: Session, map_name: str) -> dict:
        ...
        return {
            ...
            or (
                ...
                if map_name == "authenticated"
                else "Unauthenticated Checklist"
            ),
